...
void* worker(void* arg){
    ...
    for (row=a->start_row;row<a->end_row;row++){
        for (pix=0;pix<a->src->width;pix++){
            for (bit=0;bit<a->src->bpp;bit++){
                a->dest->data[Index(pix,row,a->src->width,bit,a->src->bpp)]=getPixelValue(a->src,pix,row,bit,a->alg);
            }
        }
    }
    return NULL;
...
void convolute(Image* srcImage,Image* destImage,Matrix algorithm){
    ...
    ThreadArg* args=(ThreadArg*)malloc(sizeof(ThreadArg)*threads);
    ...
    for (int t=0;t<threads;t++){
        ...
        memcpy(args[t].alg,algorithm,sizeof(Matrix));
